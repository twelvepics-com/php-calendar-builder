...
class ColorDetectorCiede2000
{
    ...
    protected function initialize(): void
    {
        ...
        foreach ($this->palette as $color => $count) {
            $labColor = ColorConverter::convertIntToLabArray($color);
            $queue->insert(
                $color,
        ...
        while ($queue->valid()) {
            ...
            $this->sortedColors[$index] = intval($queue->current());
            $queue->next();
            ++$index;
        }
    }
    ...
    protected static function mergeColors(SplFixedArray $colors, int $limit, float $maxDelta): array
    {
        ...
        if ($limit === 1) {
            if ($colors[0] === null) {
                throw new LogicException(sprintf('Unexpected color value (%s:%d).', __FILE__, __LINE__));
            }
        ...
        foreach ($colors as $color) {
            ...
            $colorLab = ColorConverter::convertIntToLabArray($color);

            foreach (array_keys($mergedColors) as $i) {
