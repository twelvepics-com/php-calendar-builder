...
class Palette implements Countable, IteratorAggregate
{
    ...
    /**
     * Get the number of the used colors from given color.
     *
     * @param int $color
     * @return int
     */
    ...
    public static function createGdImageFromFilename(string $filename, ?int $backgroundColor = null): Palette
    {
        ...
        if ($imageString === false) {
            throw new LogicException(sprintf('Unable to read image (%s:%d).', __FILE__, __LINE__));
        }

        $image = imagecreatefromstring($imageString);

        if (!$image instanceof GdImage) {
            throw new LogicException(sprintf('Unable to create GdImage entity (%s:%d).', __FILE__, __LINE__));
        }
    ...
    public static function createPaletteFromGdImage(GdImage $image, ?int $backgroundColor = null): Palette
    {
        ...
        for ($x = 0; $x < $imageWidth; ++$x) {
            for ($y = 0; $y < $imageHeight; ++$y) {
                ...
                if ($color === false) {
                    throw new LogicException(sprintf('Unable to get color (%s:%d).', __FILE__, __LINE__));
                }
